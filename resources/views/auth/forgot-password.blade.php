<x-auth-layout>
    <div class="rounded-3xl bg-white p-9 sm:p-11 shadow-2xl shadow-black/40 border border-white/60 ring-1 ring-slate-900/5">
        <div class="text-center mb-8">
            ...
            <h1 class="font-display text-3xl font-extrabold text-slate-900 tracking-tight">{{ __('Forgot your password?') }}</h1>
        ...
        <form method="POST" action="{{ route('password.email') }}" class="space-y-6">
            ...
            <div>
                ...
                <x-text-input id="email" class="block mt-2 w-full rounded-2xl border-slate-200 bg-slate-50/80 py-3.5 px-4 text-base shadow-inner focus:border-brand-500 focus:ring-brand-500" type="email" name="email" :value="old('email')" required autofocus />
        ...
        <p class="mt-8 text-center">
            <a href="{{ route('login') }}" class="text-sm font-semibold text-brand-600 hover:text-brand-800">{{ __('Already registered?') }}</a>
        </p>
    </div>
</x-auth-layout>
