...
<div {{ $attributes->class('group flex items-center gap-1.5 sm:gap-2 shrink-0') }}>
    <span class="sr-only">{{ __('brand.name') }}</span>
    @if (brand_logo_asset())
        <img src="{{ brand_logo_asset() }}" alt="{{ __('brand.name') }}" class="h-8 w-auto max-w-[10rem] object-contain shrink-0 transition duration-300 ease-out group-hover:scale-105 group-hover:drop-shadow-[0_4px_18px_rgba(37,99,235,0.45)]" />
    @else
