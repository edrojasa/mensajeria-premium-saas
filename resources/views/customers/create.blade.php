<x-app-layout>
    <x-slot name="header">
        <h2 class="font-display text-2xl font-bold text-slate-900">{{ __('customers.create_action') }}</h2>
    </x-slot>

    <div class="py-10 md:py-12">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('customers.store') }}" class="rounded-3xl border border-slate-200 bg-white shadow-2xl shadow-slate-900/10 p-8 space-y-8 ring-1 ring-slate-900/5">
                @csrf
                <div class="grid gap-6 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        <x-input-label for="name" :value="__('customers.field_name')" />
                        <x-text-input id="name" name="name" class="mt-2 block w-full rounded-2xl" :value="old('name')" required />
                        <x-input-error :messages="$errors->get('name')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="document" :value="__('customers.field_document')" />
                        <x-text-input id="document" name="document" class="mt-2 block w-full rounded-2xl" :value="old('document')" />
                    </div>
                    <div>
                        <x-input-label for="phone" :value="__('customers.field_phone')" />
                        <x-text-input id="phone" name="phone" class="mt-2 block w-full rounded-2xl" :value="old('phone')" required />
                        <x-input-error :messages="$errors->get('phone')" class="mt-2" />
                    </div>
                    <div class="sm:col-span-2">
                        <x-input-label for="email" :value="__('customers.field_email')" />
                        <x-text-input id="email" name="email" type="email" class="mt-2 block w-full rounded-2xl" :value="old('email')" />
                    </div>
                    <div class="sm:col-span-2">
                        <x-input-label for="notes" :value="__('customers.field_notes')" />
                        <textarea id="notes" name="notes" rows="3" class="mt-2 block w-full rounded-2xl border-slate-300">{{ old('notes') }}</textarea>
                    </div>
                </div>
                ...
                <fieldset class="space-y-4 rounded-2xl border border-slate-100 bg-slate-50/80 p-6">
                    ...
                    @foreach (old('addresses', [['label' => '', 'address_line' => '', 'department_id' => '', 'city_id' => '', 'reference_notes' => '', 'is_default' => false]]) as $i => $row)
                ...
                <div class="flex justify-end gap-4">
                    <a href="{{ route('customers.index') }}" class="text-sm font-semibold text-slate-600">{{ __('shipments.cancel') }}</a>
