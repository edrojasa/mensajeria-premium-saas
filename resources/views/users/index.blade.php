    ...
    <div class="py-10 md:py-12 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        @if (auth()->user()->canExportTenantReports())
            <div class="flex flex-col sm:flex-row flex-wrap gap-2 sm:gap-3 justify-end items-stretch sm:items-center">
                <x-export-link variant="primary" href="{{ route('exports.users.excel', request()->query()) }}" class="w-full sm:w-auto justify-center">{{ __('exports.excel') }}</x-export-link>
                <x-export-link href="{{ route('exports.users.pdf', request()->query()) }}" class="w-full sm:w-auto justify-center">{{ __('exports.pdf') }}</x-export-link>
            </div>
        @endif
